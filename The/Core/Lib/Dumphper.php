...
class Dumphper
{
  ...
  static function drawObject(&$source, &$parents)
  {
    ...
    $fx = function($className, $source)
    {
        echo '<span class="dumphper-key-array" style="display: inline-block; padding-right: 5px;"><span class="dumphper-key-object" style="display: inline-block; padding-right: 5px;">OBJECT</span> '.$className.'</span>';
    };

    $fxArr = function($className, $source)
    {
        echo '<span class="dumphper-key-array" style="display: inline-block; padding-right: 5px;"><span class="dumphper-key-object" style="display: inline-block; padding-right: 5px;">OBJECT</span> '.$className.'</span>';
        echo self::drawArray($source->getFieldsData());
    };

    if($source instanceof \The\Core\Application)
    {
      return $fx($className, $source);
    }elseif($source instanceof \The\Core\DB)
    {
      return $fx($className, $source);
    }elseif($source instanceof \PDOStatement)
    {
      return $fx($className, $source);
    }elseif($source instanceof \The\Core\Query)
    {
      return $fx($className, $source);
    }elseif($source instanceof \The\Core\Repository)
    {
      return $fx($className, $source);
    }elseif($source instanceof \The\Core\Model)
    {
      return $fxArr($className, $source);
    }
...
if(!defined('DPHP_USE_ACCESSIBLE')) define('DPHP_USE_ACCESSIBLE', false);
